...
/*
   Class: IverSelectTaxonomyField
   A class that initializes IverSelect Taxonomy Field
*/

class IverSelectTaxonomyField implements iIverSelectRender {
	private $type;
	private $name;
	private $label;
	private $description;
	...
	function __construct( $type, $name, $label = "", $description = "", $options = array(), $args = array(), $hidden_property = "", $hidden_values = array() ) {
		...
		add_filter( 'iver_select_taxonomy_fields', array( $this, 'addFieldForEditSave' ) );
	}
...
abstract class IverSelectTaxonomyFieldType {
	abstract public function render( $name, $label = "", $description = "", $options = array(), $args = array() );
}

class IverSelectTaxonomyFieldText extends IverSelectTaxonomyFieldType {
...
class IverSelectTaxonomyFieldImage extends IverSelectTaxonomyFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false) {
		if ( ! isset( $_GET['tag_ID'] ) ) { ?>
			<div class="form-field">
				<label for="<?php echo esc_html( $name ); ?>"><?php echo esc_html( $label ); ?></label>
				...
				<p>
					<input type="button" class="button button-secondary qodef-tax-media-add" name="qodef-tax-media-add" value="<?php esc_attr_e( 'Add Image', 'iver' ); ?>"/>
					<input type="button" class="button button-secondary qodef-tax-media-remove" name="qodef-tax-media-remove" value="<?php esc_attr_e( 'Remove Image', 'iver' ); ?>"/>
				</p>
			...
			<tr class="form-field" <?php if ($hidden) { ?> style="display: none"<?php } ?>>
				...
				<td>
					...
					<p>
						<input type="button" class="button button-secondary qodef-tax-media-add" name="qodef-tax-media-add" value="<?php esc_attr_e( 'Add Image', 'iver' ); ?>"/>
						<input data-termid="<?php echo esc_html( $_GET['tag_ID'] ); ?>" data-taxonomy="<?php echo esc_html( $taxonomy ); ?>" type="button" class="button button-secondary qodef-tax-media-remove" name="qodef-tax-media-remove" value="<?php esc_attr_e( 'Remove Image', 'iver' ); ?>"/>
					</p>
...
class IverSelectTaxonomyFieldSelect extends IverSelectTaxonomyFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {
...
class IverSelectTaxonomyFieldIcon extends IverSelectTaxonomyFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {
		...
		if ( ! isset( $_GET['tag_ID'] ) ) { ?>
			...
			<?php foreach ( $icons_collections as $icons_collection ) {
				...
				<div class="form-field qode-icon-collection-holder" style="display: none" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
					<label for="<?php echo esc_attr( $name ) . '_icon'; ?>"><?php esc_html_e( 'Icon', 'iver' ); ?></label>
			...
			<?php foreach ( $icons_collections as $icons_collection ) {
				...
				<tr class="form-field qode-icon-collection-holder" style="<?php echo esc_attr( $style ); ?>" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
					<th scope="row"><?php esc_html_e( 'Icon', 'iver' ); ?></th>
					<td>
						<select name="<?php echo esc_attr( $name . '_' . $icons_param ) ?>" id="<?php echo esc_attr( $name . '_' . $icons_param ) ?>">
							<?php
							$icons      = \IverSelectIconCollections::get_instance()->getIconCollection( $icons_collection );
...
class IverSelectTaxonomyFieldColor extends IverSelectTaxonomyFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {
...
class IverSelectTaxonomyFieldFactory {
	public function render( $field_type, $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {
		
		switch ( strtolower( $field_type ) ) {
			case 'text':
				$field = new IverSelectTaxonomyFieldText();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
			
			case 'image':
				$field = new IverSelectTaxonomyFieldImage();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
			
			case 'selectblank':
				$field = new IverSelectTaxonomyFieldSelect();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
			
			case 'icon':
				$field = new IverSelectTaxonomyFieldIcon();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
			
			case 'color':
				$field = new IverSelectTaxonomyFieldColor();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
