...
/*
   Class: IverSelectDashboardForm
   A class that initializes Select Dashboard Container
*/
class IverSelectDashboardForm implements iIverSelectLayoutNode, iIverSelectRender {
	public $children;
	public $name;
	public $form_id;
	public $form_method;
...
/*
   Class: IverSelectDashboardGroup
   A class that initializes IverSelect Group Field
*/
class IverSelectDashboardGroup implements iIverSelectLayoutNode, iIverSelectRender{
	public $children;
	public $name;
	public $title;
	public $description;
...
/*
   Class: IverSelectDashboardTitle
   A class that initializes Dashboard Title
*/
class IverSelectDashboardTitle implements iIverSelectRender {
	private $name;
	private $title;
...
/*
   Class: IverSelectDashboardField
   A class that initializes IverSelect Front Field
*/

class IverSelectDashboardField implements iIverSelectRender {
	private $type;
	private $name;
	private $label;
	private $description;
...
abstract class IverSelectDashboardFieldType {
	abstract public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = "" );
}

class IverSelectDashboardFieldText extends IverSelectDashboardFieldType {
...
class IverSelectDashboardFieldTextArea extends IverSelectDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
...
class IverSelectDashboardFieldImage extends IverSelectDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        $class = '';
			...
			<div class="qodef-dashboard-gallery-holder">
				<ul class="qodef-dashboard-gallery-images-holder">
					<?php if ( isset($value_html) ) { 
						print iver_select_get_module_part( $value_html);
					} ?>
				</ul>
				<div class="qodef-dashboard-gallery-uploader">
					...
					if ( iver_membership_theme_installed() ) {
						echo iver_select_get_button_html( array(
							'text'      => esc_html__( 'Upload', 'iver' ),
							'custom_class' => 'qodef-dashboard-gallery-upload'
						) );
					} else {
						echo '<a itemprop="url" href="#" class="qodef-btn qodef-btn-medium qodef-btn-solid qodef-dashboard-gallery-upload"><span class="qodef-btn-text">' . esc_html__( 'Upload', 'iver' ) . '</span></a>';
					} ?>
					...
					<?php if ($value !== '' && $value !== false) { ?>
						<button class="qodef-btn qodef-btn-solid qodef-dashboard-remove-image"><?php esc_html_e('Remove Media','iver'); ?></button>
					<?php } ?>
				</div>
			</div>
		</div>
...
class IverSelectDashboardFieldGallery extends IverSelectDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        $class = '';
...
class IverSelectDashboardFieldSelect extends IverSelectDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        $class = '';
...
class IverSelectDashboardFieldDate extends IverSelectFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        if (!empty($repeat) && array_key_exists('name', $repeat) && array_key_exists('index', $repeat)) {
			$id		= $name . '-' . $repeat['index'];
			$name	= $repeat['name'] . '['.$repeat['index'].']['. $name .']';
...
class IverSelectDashboardFieldIcon extends IverSelectDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$options           = \IverSelectIconCollections::get_instance()->getIconCollectionsEmpty();
		$icons_collections = \IverSelectIconCollections::get_instance()->getIconCollectionsKeys();
				...
				<div class="qodef-dashboard-item">
					...
					<div class="qodef-dashboard-icon-holder">
						...
						<?php foreach ( $icons_collections as $icons_collection ) {
							$icons_param = \IverSelectIconCollections::get_instance()->getIconCollectionParamNameByKey( $icons_collection );
	                        $style       = 'display: none';
							...
							<div class="qodef-icon-collection-holder" style="<?php echo esc_attr( $style ); ?>" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
                                <select name="<?php echo esc_attr( $name . '[' . $icons_param . ']'); ?>" id="<?php echo esc_attr( $name . '[' . $icons_param . ']'); ?>">
									<?php
									$icons = \IverSelectIconCollections::get_instance()->getIconCollection( $icons_collection );
...
class IverSelectDashboardFieldColor extends IverSelectDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
        if (!empty($repeat) && array_key_exists('name', $repeat) && array_key_exists('index', $repeat)) {
			$id		= $name . '-' . $repeat['index'];
			$name	= $repeat['name'] . '['.$repeat['index'].']['. $name .']';
...
class IverSelectDashboardFieldCheckBoxGroup extends IverSelectDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
...
class IverSelectDashboardFieldAddress extends IverSelectFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
		if ( isset( $args["col_width"] ) ) {
			$col_width = $args["col_width"];
			...
			<div class="qodef-dashboard-field-row">
				<div class="qodef-dashboard-item col-lg-<?php echo esc_attr($col_width); ?>">
					<div class="qodef-dashboard-input-holder">
						...
						<a id="reset" href="#" style="display:none;"><?php esc_html_e( 'Reset Marker', 'iver' ); ?></a>
					</div>
				</div>
			</div>
		</div>
...
class IverSelectDashboardRepeater implements iIverSelectRender {
	private $label;
	private $description;
	private $name;
	private $fields;
	...
	function __construct( $fields, $name, $label = '', $description = '', $button_text = '', $table_layout = false, $value = array() ) {
		...
		$this->name        = $name;
		$this->num_of_rows = 1;
		$this->button_text = ! empty( $button_text ) ? $button_text : esc_html__( 'Add New Item', 'iver' );
			...
			<?php if ($this->table_layout) { ?>
				<div class="qodef-dashboard-repeater-table-heading">
					<div class="qodef-dashboard-repeater-fields-holder">
						<div class="qodef-dashboard-repeater-table-cell qodef-dashboard-repeater-sort"><?php esc_html_e( 'Order', 'iver' ) ?></div>
						...
						<div class="qodef-dashboard-repeater-table-cell qodef-dashboard-repeater-remove"><?php esc_html_e( 'Remove', 'iver' ) ?></div>
					</div>
				</div>
			<?php } ?>
...
class IverSelectDashboardFieldFactory {
	public function render( $field_type, $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array()) {
		switch ( strtolower( $field_type ) ) {
			case 'text':
				$field = new IverSelectDashboardFieldText();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'textarea':
				$field = new IverSelectDashboardFieldTextArea();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'date':
				$field = new IverSelectDashboardFieldDate();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'image':
				$field = new IverSelectDashboardFieldImage();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'gallery':
				$field = new IverSelectDashboardFieldGallery();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'select':
				$field = new IverSelectDashboardFieldSelect();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'icon':
				$field = new IverSelectDashboardFieldIcon();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'color':
				$field = new IverSelectDashboardFieldColor();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'checkboxgroup':
				$field = new IverSelectDashboardFieldCheckBoxGroup();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'address':
				$field = new IverSelectDashboardFieldAddress();
