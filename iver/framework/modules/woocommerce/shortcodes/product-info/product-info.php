<?php
namespace IverCore\CPT\Shortcodes\ProductInfo;
...
class ProductInfo implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if(function_exists('vc_map')) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Product Info', 'iver' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						array(
							...
							'heading'     => esc_html__( 'Selected Product', 'iver' ),
							'settings'    => array(
								'sortable'      => true,
								'unique_values' => true
							),
							'admin_label' => true,
							'save_always' => true,
							'description' => esc_html__( 'If you left this field empty then product ID will be of the current page', 'iver' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'product_info_type',
							'heading'     => esc_html__( 'Product Info Type', 'iver' ),
							'value'       => array(
								esc_html__( 'Title', 'iver' )          => 'title',
								esc_html__( 'Featured Image', 'iver' ) => 'featured_image',
								esc_html__( 'Category', 'iver' )       => 'category',
								esc_html__( 'Excerpt', 'iver' )        => 'excerpt',
								esc_html__( 'Price', 'iver' )          => 'price',
								esc_html__( 'Rating', 'iver' )         => 'rating',
								esc_html__( 'Add To Cart', 'iver' )    => 'add_to_cart',
								esc_html__( 'Tag', 'iver' )            => 'tag',
								esc_html__( 'Author', 'iver' )         => 'author',
								esc_html__( 'Date', 'iver' )           => 'date',
							),
							'admin_label' => true
						),
						array(
							...
							'heading'    => esc_html__( 'Product Info Color', 'iver' ),
						...
						array(
							...
							'value'       => array_flip( iver_select_get_title_tag( true, array( 'p' => 'p' ) ) ),
							'description' => esc_html__( 'Set title tag for product title element', 'iver' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Featured Image Proportions', 'iver' ),
							'value'      => array(
						...
						array(
							...
							'heading'     => esc_html__( 'Category Tag', 'iver' ),
							'value'       => array_flip( iver_select_get_title_tag( true ) ),
							'description' => esc_html__( 'Set category tag for product category element', 'iver' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Add To Cart Skin', 'iver' ),
							'value'      => array(
								esc_html__( 'Default', 'iver' ) => '',
								esc_html__( 'White', 'iver' )   => 'white',
						...
						array(
							...
							'heading'    => esc_html__( 'Add To Cart Size', 'iver' ),
							'value'      => array(
								esc_html__( 'Default', 'iver' ) => '',
							...
							'dependency' => array( 'element' => 'product_info_type', 'value'   => array( 'iver' ) ),
							'group'      => esc_html__( 'Product Info Style', 'iver' )
						)
					)
				)
	...
	public function getItemTitleHtml( $params ) {
		...
		if ( ! empty( $title ) ) {
			$html = '<' . esc_attr( $title_tag ) . ' itemprop="name" class="qodef-pi-title entry-title" ' . iver_select_get_inline_style( $product_info_style ) . '>';
			...
			$html .= '<div class="qodef-pi-category">';
				foreach ( $categories as $cat ) {
					if ( ! empty( $params['category_tag'] ) ) {
						$html .= '<' . esc_attr( $params['category_tag'] ) . ' ' . iver_select_get_inline_style( $product_info_style ) . '>';
						...
						$html .= '<a itemprop="url" class="qodef-pi-category-item" href="' . esc_url( get_term_link( $cat->term_id ) ) . '" ' . iver_select_get_inline_style( $product_info_style ) . '>' . esc_html( $cat->name ) . '</a>';
					}
				}
			$html .= '</div>';
		}
	...
	public function getItemExcerptHtml( $params ) {
		...
		if ( ! empty( $excerpt ) ) {
			$html = '<div class="qodef-pi-excerpt"><p itemprop="description" class="qodef-pi-excerpt-item" ' . iver_select_get_inline_style( $product_info_style ) . '>' . esc_html( $excerpt ) . '</p></div>';
		}
	...
	public function getItemPriceHtml( $params ) {
		...
		if ( $price_html = $product->get_price_html() ) {
			$html = '<div class="qodef-pi-price" ' . iver_select_get_inline_style( $product_info_style ) . '>' . $price_html . '</div>';
		}
	...
	public function getItemRatingHtml( $params ) {
		...
		if ( get_option( 'woocommerce_enable_review_rating' ) !== 'no' ) {
			...
			$html = '<div class="qodef-pi-rating" title="' . sprintf( esc_html__( "Rated %s out of 5", "iver" ), $average ) . '" ' . iver_select_get_inline_style( $product_info_style ) . '><span style="width:' . ( ( $average / 5 ) * 100 ) . '%"></span></div>';
		}
	...
	public function getItemAddToCartHtml( $params ) {
		...
		$html .= apply_filters( 'iver_select_product_info_add_to_cart_link',
	...
	public function getItemTagHtml( $params ) {
		...
		if ( ! empty( $tags ) ) {
			$html = '<div class="qodef-pi-tag">';
				foreach ( $tags as $tag ) {
					$html .= '<a itemprop="url" class="qodef-pi-tag-item" href="' . esc_url( get_term_link( $tag->term_id ) ) . '" ' . iver_select_get_inline_style( $product_info_style ) . '>' . esc_html( $tag->name ) . '</a>';
				}
			$html .= '</div>';
		}
	...
	public function getItemAuthorHtml( $params ) {
		...
		if ( ! empty( $author ) ) {
			$html = '<div class="qodef-pi-author" ' . iver_select_get_inline_style( $product_info_style ) . '>' . esc_html( $author ) . '</div>';
		}
	...
	public function getItemDateHtml( $params ) {
		...
		if ( ! empty( $date ) ) {
			$html = '<div class="qodef-pi-date" ' . iver_select_get_inline_style( $product_info_style ) . '>' . esc_html( $date ) . '</div>';
		}
	...
	public function productIdAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				$data = array();
				$data['value'] = $value['id'];
				$data['label'] = esc_html__( 'Id', 'iver' ) . ': ' . $value['id'] . ( ( strlen( $value['title'] ) > 0 ) ? ' - ' . esc_html__( 'Title', 'iver' ) . ': ' . $value['title'] : '' );
				$results[] = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( ! is_wp_error( $product ) ) {
				...
				if ( ! empty( $product_title ) ) {
					$product_title_display = ' - ' . esc_html__( 'Title', 'iver' ) . ': ' . $product_title;
				}
				
				$product_id_display = esc_html__( 'Id', 'iver' ) . ': ' . $product_id;
