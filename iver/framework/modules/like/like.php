...
class IverSelectLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_iver_select_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_iver_select_like', array( $this, 'ajax' ) );
	}
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'iver' );
		
		if ( isset( $_COOKIE[ 'qodef-like_' . $post->ID ] ) ) {
			$class = 'qodef-like liked';
			$title = esc_html__( 'You already like this!', 'iver' );
		}
...
if ( ! function_exists( 'iver_select_create_like' ) ) {
	function iver_select_create_like() {
		IverSelectLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'iver_select_create_like' );
}