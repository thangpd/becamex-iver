...
namespace Iver\Modules\Header\Lib;
/**
 * Class IverSelectHeaderConnector
 *
...
class IverSelectHeaderConnector {
	...
	public function connect( $config = array() ) {
		do_action( 'iver_select_pre_header_connect' );
		...
		if ( ! empty( $config['affect_content'] ) ) {
			add_filter( 'iver_select_content_elem_style_attr', array( $this, 'contentMarginFilter' ) );
		}
		
		if ( ! empty( $config['affect_title'] ) ) {
			add_filter( 'iver_select_title_content_padding', array( $this, 'titlePaddingFilter' ) );
		}
		
		do_action( 'iver_select_after_header_connect' );
	}
	 ...
	 *
	 * Hooks to iver_select_title_content_padding filter
	 *
	 * @return int
	 */
